...
public class CuttingGuideGenerator implements GuideGUIGenerator {
    private static final ItemStack ICON = Registries.ITEM.getItemStack(Keys.cutting_board);
    ...
    public @NotNull Inventory recipeToInventory(@NotNull ArtisanRecipe artisanRecipe, @NotNull InventoryHolder inventoryHolder, @NotNull Component component) {
        final CuttingBoardRecipe cuttingBoardRecipe = (CuttingBoardRecipe) artisanRecipe;
        final Inventory inventory = Bukkit.createInventory(inventoryHolder, 27, component);
        inventory.setItem(10, ICON);
        final List<NamespacedKey> input = new ArrayList<>();
        final List<Choice> handle = new ArrayList<>();
        handle.add(cuttingBoardRecipe.getInput());
        ...
        for (int i = 0; i < 3; i++) {
            if (i >= cuttingBoardRecipe.getInputs().size()) break;
            inventory.setItem(14 + i, cuttingBoardRecipe.getResultGenerators().get(i).generate());
        }
        return inventory;
    }
}
